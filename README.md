...
# TJ Sylva · 知识之林 / Xuenwu 学生端学习系统

**左侧 AI 对话、右侧自动生长的结构化知识树，并扩展了学生端 AI 学习助手、智能练习和错题本**
...
>
> 本仓库在原 TJ Sylva 的基础上增加了 **Xuenwu 学生端学习系统**。它不替代原来的知识地图学习流程，而是在学生已有学习地图上提供 AI 学习建议、考试风格智能练习、错题记录、学习反思和下一步建议。
...
Xuenwu 是我在 TJ Sylva 基础上继续学习和改造出来的学生端功能。它的目标不是重新做一个课程平台，而是围绕已有知识地图补上“练习、错题、复习建议”的学习闭环：

- 学生平时仍然在 TJ Sylva 里创建知识地图、按节点学习。
- Xuenwu 会根据当前选中的学习地图生成考试风格练习题。
- 高数、四级、Java 等不同学习地图通过 `session_id` 分开记录，避免复习内容混在一起。
- 左侧菜单新增 `AI 学习助手`、`智能练习`、`错题本` 三个入口，不使用浮窗，尽量保持和原项目界面一致。
- `AI 学习助手` 负责学习调度和建议，可以引导学生去做练习或查看错题。
- `智能练习` 负责根据当前学习路线和知识节点生成题目，题型倾向于选择题、填空题、计算/解答题和应用题。
- `错题本` 负责保存做错的题目、学生答案、参考答案和后续反思。
- 当前版本已经新增复习内容、作答记录、错题本、学习反思、助手建议历史等数据库表。

目前 Xuenwu 的智能练习已经接入项目原有的 OpenAI 兼容 LLM 配置，优先由 AI 根据当前学习地图生成考试风格练习题；如果 LLM 未配置或调用失败，会回退到本地考试题模板。学生在智能练习中填写答案后，可以手动标记“做对了 / 做错了”，做错的题会自动进入错题本。
...
| GET | `/api/xuenwu/sessions/{session_id}/review-items` | 读取当前学习地图的练习题记录 |
| POST | `/api/xuenwu/sessions/{session_id}/review-items/generate` | 根据当前学习路线生成考试风格练习题 |
| POST | `/api/xuenwu/review-items/{review_item_id}/attempts` | 提交练习作答，答错自动进入错题本 |
| GET | `/api/xuenwu/sessions/{session_id}/wrong-questions` | 读取当前学习地图的错题 |
| POST | `/api/xuenwu/reflections` | 保存学习反思 |
| GET | `/api/xuenwu/sessions/{session_id}/suggestions` | 读取学习助手建议历史 |
| POST | `/api/xuenwu/suggestions` | 保存学习助手建议 |
...
- [app/routers/xuenwu.py](app/routers/xuenwu.py) — Xuenwu 学习助手、智能练习、错题本接口
- [app/schemas/xuenwu.py](app/schemas/xuenwu.py) — Xuenwu 请求/响应字段定义

任何接口/字段变化都先动 schemas + Alembic，再改 service + router，最后联前端。

## 项目来源说明

本项目基于 [nitianji415/TJ-Sylva](https://github.com/nitianji415/TJ-Sylva) 继续学习和二次开发。原项目由 GitHub 用户 `nitianji415` 开源，我在其基础上尝试加入 Xuenwu AI 学习助手功能，用来练习完整项目开发、前后端联调、数据库设计和 Docker 部署。

我目前还是一名大学生，项目能力和工程经验都还在学习阶段。这个仓库主要用于课程项目、个人学习和功能实验。如果代码中有不完善的地方，也欢迎指出。
