...
# TJ Sylva · 知识之林 / Xuenwu 学习助手

**左侧 AI 对话、右侧自动生长的结构化知识树，并扩展了学生端 AI 学习助手**
...
>
> 本仓库在原 TJ Sylva 的基础上增加了 **Xuenwu AI 学习助手**。它不替代原来的知识地图学习流程，而是在学生已有学习地图上提供轻量复习、错题记录、学习反思和下一步建议。
...
Xuenwu 是我在 TJ Sylva 基础上继续学习和改造出来的学生端功能。它的目标不是重新做一个课程平台，而是解决学习过程中的复习问题：

- 学生平时仍然在 TJ Sylva 里创建知识地图、按节点学习。
- Xuenwu 会根据当前选中的学习地图生成轻量复习内容。
- 高数、四级、Java 等不同学习地图通过 `session_id` 分开记录，避免复习内容混在一起。
- 左侧菜单新增 `AI 学习助手` 入口，不使用浮窗，尽量保持和原项目界面一致。
- 学习助手提供三个入口：`开始今日轻量复习`、`回顾最近知识节点`、`查看错题与反思`。
- 当前版本已经新增复习内容、作答记录、错题本、学习反思、助手建议历史等数据库表。

目前 Xuenwu 的复习内容生成还处在第一版：已经完成页面入口、数据库表、后端接口和基础前端调用，后续会继续把本地占位复习内容替换成真正结合知识节点、学习记录和错题记录的 AI 生成。
...
| GET | `/api/xuenwu/sessions/{session_id}/review-items` | 读取当前学习地图的复习内容 |
| POST | `/api/xuenwu/sessions/{session_id}/review-items/generate` | 生成轻量复习内容 |
| POST | `/api/xuenwu/review-items/{review_item_id}/attempts` | 提交复习作答，答错自动进入错题本 |
...
- [app/routers/xuenwu.py](app/routers/xuenwu.py) — Xuenwu AI 学习助手接口
